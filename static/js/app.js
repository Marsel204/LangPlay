/**
 * LinguaPlay — Main Application Entry Point (app.js)
 * Bootstraps modules, coordinates UI events, subtitle sync, video streaming,
 * multi-provider AI explanations (Antigravity CLI / Gemini / OpenRouter), and Anki bookmarking.
 */
...
// ══════════════════════════════════════════════════════════════════════
//  1. Subtitle Synchronization Callback
// ══════════════════════════════════════════════════════════════════════
...
function updateAIProviderUI() {
  ...
  if (prov === 'antigravity') {
    ...
    if (aiApiKeyInput) {
      ...
      const savedKey = getSavedApiKey(prov);
...
// ══════════════════════════════════════════════════════════════════════
//  3. YouTube URL / Search / Caption Flow
// ══════════════════════════════════════════════════════════════════════
...
async function fetchServerCaptions(videoId) {
  try {
    ...
    console.warn('[Captions] Backend fetch failed:', e);
    return null;
  }
}
...
async function loadYouTube(input) {
  ...
  try {
    await loadYouTubeVideo(videoId, async (errorCode, vidId) => {
      if (errorCode === 101 || errorCode === 150) {
        showYoutubeFeedback("Embedding disabled by owner. Attempting direct stream extraction via local server...");
        try {
          const sRes = await fetch(`/api/stream?v=${encodeURIComponent(vidId)}`);
          if (sRes.ok) {
            const sData = await sRes.json();
            if (sData.url) {
              showNativeUI();
              videoPlayer.src = sData.url;
              videoPlayer.load();
              videoPlayer.play().catch(e => console.warn('Autoplay prevented:', e));
              clearYoutubeFeedback();
              showToast('Direct stream loaded (bypassed embed restriction)!', 'success');
              return;
            }
          }
        } catch (sErr) {
          console.error('Stream fallback error:', sErr);
        }
      }
      showYoutubeFeedback(`YouTube playback error (${errorCode}). If embedding is blocked, try another video.`);
    });
  } catch (err) {
  ...
  if (cues && cues.length > 0) {
    ...
    showToast('No auto subtitles found. You can upload a .srt/.vtt file manually.', 'warn', 4000);
    if (captionsYtdlpCmd) {
...
async function executeYouTubeSearch(query) {
  ...
  try {
    ...
    renderSearchResults(videos, ytSearchResultsContainer, (selected, isFolder) => {
      if (isFolder && (selected.url || selected.id)) {
        let folderUrl = selected.url || selected.id;
        if ((folderUrl.includes('/channel/') || folderUrl.includes('/@')) && !folderUrl.includes('/playlists') && !folderUrl.includes('/videos')) {
          folderUrl = folderUrl.replace(/\/$/, '') + '/videos';
        }
        youtubeUrlInput.value = folderUrl;
        executeYouTubeSearch(folderUrl);
      } else {
        ...
        if (targetId) {
          ...
          youtubeUrlInput.value = selected.url;
          loadYouTube(selected.url);
        }
      }
    });
  } catch (e) {
    ytSearchResultsContainer.innerHTML = `
      <div class="text-center py-12 text-rose-400 text-xs">
        Search failed: ${e.message}. Ensure the local server is running.
      </div>
    `;
...
function handleTokenClick(tokenEl) {
  ...
  activePos.textContent = `Part of Speech: ${pos || '—'} (${posDetail || '—'}) • Base: ${baseform}`;
  ...
  // Reset AI & Anki states in drawer
  abortAIAnalysis();
...
export async function initApp() {
  ...
  initTokenizer((status, msg) => {
    if (status === 'loading') {
      ...
      console.log('[LinguaPlay] Morphological analyzer ready.');
    } else if (status === 'error') {
      videoLabel.textContent = '⚠ Dict load failed';
      showToast(msg, 'error');
    }
  });

  // 3. Check Antigravity CLI and setup AI Provider
  const agyStatus = await checkAntigravityStatus();
  if (agyStatus.available) {
    console.log('[AI] Antigravity CLI detected at:', agyStatus.path);
  }
  updateAIProviderUI();
  ...
  // 6. Subtitle Overlay Token Click
  ...
  // 10. Ask AI (Supports Antigravity CLI / Gemini / OpenRouter)
  ...
  // 11. Quick Add to Anki (Dual Pipeline)
  quickAnkiBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      quickAnkiBtn.className = 'w-full py-2.5 px-4 rounded-xl text-sm font-semibold text-amber-400 bg-amber-500/10 border border-amber-500/30 hover:bg-amber-500/20 transition-all flex items-center justify-center gap-2';
  ...
  console.log('🚀 LinguaPlay modular client initialized.');
}
