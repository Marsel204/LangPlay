/**
 * LinguaPlay — Dictionary Module (dict.js)
 * Provides built-in offline vocabulary lookups and online Google Translate fallback.
 */
 ...
 * @param {string} word - The term or base form to look up
...
export async function fetchGoogleTranslation(word) {
  ...
  try {
    ...
    if (dictDefs.length > 0) {
      result += ` — <span class="text-slate-400 italic text-xs">(${dictDefs.join(' | ')})</span>`;
    }
